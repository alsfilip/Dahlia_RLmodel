functions{
	real RL_log(int[] chosen, int[] unchosen, int[] action, int[] sub, int[] trial, int[] f1_side, int[] f2_side, vector reward, int N,vector alpha_f, vector beta_f,vector alpha_a, vector beta_a, vector C_f, vector C_a, vector D){
    matrix[1,2] Qf;
    matrix[1,2] Qa;
    real probf1;
    vector[N] lprob;
    ...
    for(i in 1:N){
      ...
      d = D[sub[i]];                       // Bias towards fractal 1
      ...
      probf1 = 1/(1+exp(-(sm_f*(Qf[1,1]-Qf[1,2])+sm_a*(Qa[1,f1_side[i]]-Qa[1,f2_side[i]])+c_f*cf_1+c_a*ca_1+d)));
...
model{
  ...
  chosen~RL(unchosen, action, sub, trial, f1_side, f2_side, reward, N, alpha_f, beta_f, alpha_a, beta_a, C_f, C_a, D);
}